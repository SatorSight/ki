...
class Image324OptimizeCommand extends ContainerAwareCommand
{
    protected function configure()
    {
        //todo make adminka

        $this->setName('dev:images-optimize-324')
    ...
    protected function execute(InputInterface $input, OutputInterface $output)
    {
        ...
        foreach ($journals as $j){
            ...
            if(is_file($path.$image)) {
                ...
                $image_final_name = $path . str_replace($image_name, '324_' . $image_name, $image);

                $img->writeImage($image_new_name);

                exec('cd vendor/mozjpeg && ./cjpeg -quality 70 -outfile ' . $image_final_name . ' ' . $image_new_name);
